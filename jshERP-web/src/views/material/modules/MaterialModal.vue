    ...
    onBarCodeChange(checked) {
      ...
      if (unit) {
        if (checked) {
          //计算多属性已经选择了几个
          let count =
            this.getNumByField('manyColor') +
            this.getNumByField('manySize') +
            this.getNumByField('other1') +
            this.getNumByField('other2') +
            this.getNumByField('other3')
          if (count === 2) {
            let skuArr = []
    ...
    batchSetStockModalFormOk(stock, batchType) {
      ...
      for (let i = 0; i < arr.length; i++) {
        let depotInfo = {
          ...
          initStockCost: arr[i].initStockCost,
